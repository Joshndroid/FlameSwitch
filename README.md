...
The app contacts only the Home Assistant address you configure and refreshes readings about once a minute. The token is kept in `data/home-assistant.json` on the server and is never returned to the browser after saving. Selected readings are visible to everyone who can view the homepage. Forecast availability depends on the selected Home Assistant entities; Home Assistant integrations may make their own external requests.
